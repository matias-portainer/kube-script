...
main() {
    ...
    [[ "$(command -v microk8s kubectl)" ]] || errorAndExit "Unable to find microk8s kubectl binary. Please ensure microk8s kubectl is installed before running this script."
    
    info "Downloading agent manifest..."
    curl -L  https://downloads.portainer.io/ee2-19/portainer-agent-edge-k8s.yaml -o portainer-agent-edge-k8s.yaml || errorAndExit "Unable to download agent manifest"
